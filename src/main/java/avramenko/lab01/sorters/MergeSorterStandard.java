 ...
 */
public class MergeSorterStandard extends MergeSorter implements Runnable{

    private int[] array;
    private int recursiveLevel;
    private int[] result;

    public MergeSorterStandard() {
    }

    public MergeSorterStandard(int[] array, int recursiveLevel, int[] result) {
        this.array = array;
    ...
    public void mergeSort(int[] array, int recursiveLevel) throws InterruptedException {
        ...
        if (Runtime.getRuntime().availableProcessors() > threadCount) {
            ...
            Runnable runnable1 = new MergeSorterStandard(tempArray1, recursiveLevel, result1);
            Runnable runnable2 = new MergeSorterStandard(tempArray2, recursiveLevel, result2);
            ...
            int[] tempResult = standardSorter.sort(array);
            for (int i = 0; i < tempResult.length; i++){
                this.result[i] = tempResult[i];
            }
        }
