...
/**
 * <h1>MergeSorterBubbleFromBeginning</h1>
 * <p>The MergeSorterBubbleFromBeginning is a child of abstract class {@link MergeSorter}.
 ...
 */
public class MergeSorterBubbleFromBeginning extends MergeSorter implements Runnable {
    private int[] array;
    private int recursiveLevel;
    private int[] result;

    public MergeSorterBubbleFromBeginning() {
    }

    public MergeSorterBubbleFromBeginning(int[] array, int recursiveLevel, int[] result) {
        this.array = array;
     ...
     * {@link BubbleSorterFromBeginning#sort(int[])} and then {@link MergeSorter#merge(int[], int[], int, int)}
    ...
    public void mergeSort(int[] array, int recursiveLevel) throws InterruptedException {
        ...
        if (Runtime.getRuntime().availableProcessors() > threadCount) {
            ...
            Runnable runnable1 = new MergeSorterBubbleFromBeginning(tempArray1, recursiveLevel, result1);
            Runnable runnable2 = new MergeSorterBubbleFromBeginning(tempArray2, recursiveLevel, result2);
            ...
            int[] tempResult = bubbleSorterFromBeginning.sort(array);
            for (int i = 0; i < tempResult.length; i++){
                this.result[i] = tempResult[i];
            }
        }
